...
class MongoService {
    ...
    findOne(collectionName, query, fields) {
        return this.connect().then(db => {
            let result = db.collection(collectionName).findOne(query, fields);
            if (!this.$config.verbose) return result;
                
            console.log("db." + collectionName + ".findOne(" + JSON.stringify(query) + ", " + JSON.stringify(fields) + ") => ", JSON.stringify(result));
            console.log();
            return result;
        });
    };
