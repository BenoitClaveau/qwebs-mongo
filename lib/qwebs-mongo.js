...
class MongoService {
    ...
    insert(collectionName, item) {
        return this.connect().then(db => {
            return db.collection(collectionName).insert(item).then(result => {
                if (!this.$config.verbose) return result.ops[0];
                
                console.log("db." + collectionName + ".insert(", item, ")");
                console.log();
                return result.ops[0];
            });
        });
    ...
    remove(collectionName, selector) {
        return this.connect().then(db => {
            return db.collection(collectionName).remove(selector).then(result => {
                ...
                console.log("db." + collectionName + ".remove(", selector, ")");
                console.log();
                return result;
            });
        });
    ...
    update(collectionName, query, update, options) {
        return this.connect().then(db => {
            ...
            return db.collection(collectionName).update(query, copy, options).then(result => {
                ...
                console.log("db." + collectionName + ".update(", query, ",", copy, ",", options, ") => ", result, " element(s).");
                console.log();
                return result;
            });
        });
    ...
    updateOne(collectionName, filter, update, options) {
        return this.connect().then(db => {
            ...
            return db.collection(collectionName).update(filter, copy, options).then(result => {
                ...
                console.log("db." + collectionName + ".update(", filter, ",", copy, ",", options, ") => " + res[0] + " element(s).");
                console.log();
                return result;
            });
        });
    ...
    updateMany(collectionName, filter, update, options) {
        return this.connect().then(db => {
            ...
            return db.collection(collectionName).updateMany(filter, copy, options).then(result => {
                ...
                console.log("db." + collectionName + ".updateMany(", filter, ",", copy, ",", options, ") => " + res[0] + " element(s).");
                console.log();
                return result;
            });
        });
    ...
    findOne(collectionName, query, fields) {
        return this.connect().then(db => {
            ...
            return db.collection(collectionName).findOne(query, fields).then(result => {
                ...
                console.log("db." + collectionName + ".findOne(", query, ",", fields,") => ", result);
                console.log();
                return result;
            });
        });
    ...
    find(collectionName, query, options) {
        return this.connect().then(db => {
            ...
            return cursor.count(true).then(count => {
                console.log("db." + collectionName + ".find(", query, ",", options, ") => " + count + " elements.");
                console.log();
                cursor.rewind();
                return cursor;
            });
    ...
    find2(collectionName, query, meta, options) {
        return this.connect().then(db => {
            ...
            let cursor = db.collection(collectionName).find(query, meta, options);
            ...
            return cursor.count(true).then(count => {
                console.log("db." + collectionName + ".find(", query, ",", meta, ",", options, ").skip(" + skip + ").limit(" + limit + ") => " + count + " elements.");
                console.log();
                cursor.rewind();
                return cursor;
            });
    ...
    count(collectionName, query, options) {
        return this.connect().then(db => {
            ...
            let cursor = db.collection(collectionName).find(query, options);
            if (!this.$config.verbose) return cursor.count(true);
            
            let count = cursor.count(true);
            console.log("db." + collectionName + ".find(", query, ",", options, ") => " + count + " elements.");
            console.log();
            return count;
        });
    ...
    geoNear(collectionName, x, y, options) {
        return this.connect().then(db => {
            ...
            return db.command(command).then(doc => {
                if (this.$config.verbose) console.log("db.runCommand(", command, ") => " + doc.results.length + " element(s).");
                return doc;
            });
        });
    };
    ...
    aggregate(collectionName, array) {
        return this.connect().then(db => {
            return db.collection(collectionName).aggregate(array).then(result => {
                ...
                console.log("db." + collectionName + ".aggregate(", array, ")");
                console.log();
                return result;
            });
        });
    ...
    mapReduce(collectionName, map, reduce, options) {
        return this.connect().then(db => {
            return db.collection(collectionName).mapReduce(map, reduce, options).then(result => {
                ...
                console.log("db." + collectionName + ".mapReduce(" + map + "," + reduce + ",", options, ")");
                console.log();
                return result;
            });
        });
    ...
    gridStore(objectId, mode, options) {
        return this.connect().then(db => {
            ...
            if (this.$config.verbose) { 
                console.log("new GridStore(db, " + objectId + "," + mode + ",", options, ")");
                console.log();
            }
    ...
    gridFSBucket(options) {
        return this.connect().then(db => {
            options = options || {};
            if (this.$config.verbose) { 
                console.log("new GridFSBucket(db, ", options, ")");
                console.log();
            }
    ...
    ensureIndex(collectionName, index, options) {
        return this.connect().then(db => {
            ...
            options = options || {};
            return db.collection(collectionName).ensureIndex(index, options).then(result => {
                if (!this.$config.verbose) return result;
                
                console.log("db." + collectionName + ".ensureIndex(", index, ",", options, ")");
                console.log();
                return result;
            });
        });
    ...
    dropIndex(collectionName, index) {
        return this.connect().then(db => {
            ...
            return db.collection(collectionName).dropIndex(index).then(result => {
                if (!this.$config.verbose) return result;
                
                console.log("db." + collectionName + ".dropIndex(" + index + ")");
                console.log();
                return result;
            });
        });
