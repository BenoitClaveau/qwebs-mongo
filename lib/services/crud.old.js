...
class CrudService {
    ...
    httpGet(request, response) {
        ...
        return this.mongoGet(query, options).then(content => {
            return response.send({ request: request, content: content });
        });
    };

    httpInsert(request, response) {
        if (!request.body) throw new Error("Request body is not defined.");
    ...
    httpUpdateOne(request, response) {
        const {filter, update, options} = request.mongo || {};
        return this.mongoUpdateOne(filter, update, options).then(content => {
            return response.send({ request: request, content: content });
        });
    };

    httpUpdateMany(request, response) {
        const {filter, update, options} = request.mongo || {};
        return this.mongoUpdateMany(filter, update, options).then(content => {
            return response.send({ request: request, content: content });
        });
    };
    ...
    httpDeleteOne(request, response) {
        const {filter, options} = request.mongo || {};
        return this.mongoDeleteOne(filter, options).then(content => {
            return response.send({ request: request, content: content });
        });
    };
    ...
    httpDeleteMany(request, response) {
        const {filter, options} = request.mongo || {};
        return this.mongoDeleteMany(filter, options).then(content => {
            return response.send({ request: request, content: content });
        });
    };
    ...
    mongoGetById(id, options) {
        return this.collection.then(collection => {
            ...
            return collection.findOne({ _id: id }, options).then(object => {
                if (!object) throw new Error("Object not found in ${objectName}.", { objectName: objectName });
                return object;
            });
        });
    };
    ...
    mongoSave(object) {
        return this.mongoGetById(object._id).then(previous => {
            ...
            if (error.template != "Object not found in ${objectName}.") throw error;
    ...
    mongoUpdateOne(filter, update, options) {
        return this.collection.then(collection => {
            return collection.updateOne(filter, update, options).then(res => {
                return this.mongoGet(filter);
            });
        });
    };

    mongoUpdateMany(filter, update, options) {
        return this.collection.then(collection => {
            return collection.mongoUpdateMany(filter, update, options).then(res => {
                return this.mongoArray(filter);
            });
        });
    };
    ...
    mongoDeleteOne(filter, options) {
        return this.collection.then(collection => {
            if (!filter) throw new Error("Filter isn't defined.");
            return collection.deleteOne(filter, options);
        });
    };

    mongoDeleteMany(filter, options) {
        return this.collection.then(collection => {
            if (!filter) throw new Error("Filter isn't defined.");
