...
class GridFSService extends CRUD {
    constructor(bucketName, $mongo) {
        super(`${bucketName}.files`, $mongo);

        if (!bucketName) throw new DataError({ message: "bucketName isn't defined." });
    ...
    donwloadByName(request, response) {
        ...
        if (!filename) throw new DataError({ message: "Filename is not defined." });
    ...
    uploadDataURL(request, response) {
        ...
        if (!match) throw new DataError({ message: "Body is not a dataURL." });
        ...
        return this.fileUpload(filename, buffer, contentType).then(file => {
    ...
    get bucket() {
        return new Promise((resolve, reject) => {
            ...
            else return this.$mongo.db.then(db => {
    ...
    openUploadStream(filename, options) { 
       return this.bucket.then(bucket => {
         if (!filename) throw new DataError({ message: "Filename isn't defined." });
    ...
    renameFile(id, name) { 
      return this.bucket.then(bucket => {
        if (!id) throw new DataError({ message: "Id isn't defined." });
        if (!name) throw new DataError({ message: "Name isn't defined." });
        if (id instanceof ObjectID == false) id = new ObjectID(id);
