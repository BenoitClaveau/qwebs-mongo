...
class CrudService {
    ...
    mongoGetById(id, options) {
        return this.collection.then(collection => {
            ...
            return collection.findOne({ _id: id }, options).then(object => {
                if (!object) throw new DataError({ message: `Object not found in ${this.objectName}.`, data: { id: id }});
                return object;
            });
        });
    };
    ...
    mongoGet(query, options) {
        return this.collection.then(collection => {
            ...
            return collection.findOne(query, options).then(object => {
                if (!object) throw new DataError({ message: `Object not found in ${this.objectName}.`, data: { query: query }});
                return object;
            });
        });
    };
    ...
    mongoSave(object) {
        return this.mongoGetById(object._id).then(previous => {
            ...
            if (error.message != `Id isn't defined.`) throw error;
