...
class CrudService {
    ...
    insert(options) {  
        return streamify(async (chunk, enc) => {
            const res = await this.mongoInsertOne(chunk, options);
    ...
    update(filter, options) {
        return streamify(async (chunk, enc) => {
            const { _id, ...doc } = chunk;
            const res = await this.mongoUpdateOne(filter || { _id }, doc, options);
    ...
    replace(filter, options) {
        return streamify(async (chunk, enc) => {
            const { _id, ...doc } = chunk;
            const res = await this.mongoReplaceOne(filter || { _id }, doc, options);
            if (!res) throw new Error("Fail to replace document.", { document: chunk });
            return res.ops[0];
        })
    }
    ...
    delete(filter, options) {
        return streamify(async (chunk, enc) => {
            const { _id } = chunk;
            const res = await this.mongoDeleteOne(filter || { _id }, options);
